...
private:
    ...
    void sendGripperCommand(float position, float force, float speed, bool skip_init_check = false)
    {
        ...
        if (pos_success) {
            RCLCPP_INFO(this->get_logger(), "✅ Position register write SUCCESS");
        } else {
            RCLCPP_ERROR(this->get_logger(), "❌ Position register write FAILED");
        }
        ...
        if (force_success) {
            RCLCPP_INFO(this->get_logger(), "✅ Force register write SUCCESS");
        } else {
            RCLCPP_ERROR(this->get_logger(), "❌ Force register write FAILED");
        }
        ...
        if (speed_success) {
            RCLCPP_INFO(this->get_logger(), "✅ Speed register write SUCCESS");
        } else {
            RCLCPP_ERROR(this->get_logger(), "❌ Speed register write FAILED");
        }
        
        // 总结结果
        bool overall_success = pos_success && force_success && speed_success;
        
        RCLCPP_INFO(this->get_logger(), "📊 Register write summary:");
        ...
        if (overall_success) {
            ...
            if (pos_success) {
                gripper_state_.target_position = position;
                RCLCPP_INFO(this->get_logger(), "📍 Position command applied despite other failures");
            }
            if (force_success) {
                gripper_state_.target_force = force;
                RCLCPP_INFO(this->get_logger(), "💪 Force command applied despite other failures");
            }
        }
    }
    ...
    void publishJointState()
    {
        ...
        joint_state.header.frame_id = "ag95_base_link";

        // 只发布主关节
        joint_state.name = {"left_outer_knuckle_joint"};

        // 计算主关节角度
        // 假设 gripper_state_.position 是当前开口（单位mm），max_position_ 是最大开口（单位mm）
        double closed_angle = 0.93; // 最大合拢角度
